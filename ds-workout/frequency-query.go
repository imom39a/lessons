...
func createIndexMapping(dataStoreIndex map[int32]map[int32]struct{}, d, frequencyOfCurrentItem, frequencyOfNextItem int32){    
    delete(dataStoreIndex[frequencyOfCurrentItem],d)
    _, found := dataStoreIndex[frequencyOfNextItem]
    if !found {
        dataStoreIndex[frequencyOfNextItem] = make(map[int32]struct{})
    }
    dataStoreIndex[frequencyOfNextItem][d] = struct{}{}
}

func freqQuery(queries [][]int32) []int32 {
    
    queryResult := make([]int32,0)
    dataStore := make(map[int32]int32)
    dataStoreIndex := make(map[int32]map[int32]struct{})

    for i:= range queries{
        operation := queries[i][0]
        data := queries[i][1]
        if operation == 1 {
            frequencyOfCurrentItem := dataStore[data]
            frequencyOfNextItem := dataStore[data] + 1
            createIndexMapping(dataStoreIndex, data, frequencyOfCurrentItem, frequencyOfNextItem)
            dataStore[data]++
        } else if operation == 2 {
            v, found := dataStore[data]
            frequencyOfCurrentItem := dataStore[data]
            frequencyOfNextItem := dataStore[data] - 1
            if found && v > int32(0) {
                createIndexMapping(dataStoreIndex, data, frequencyOfCurrentItem, frequencyOfNextItem)
            ...
            v := int32(0)
            _, found := dataStoreIndex[data]
            if found && len(dataStoreIndex[data]) > 0 {
                v = int32(1)
            }            
